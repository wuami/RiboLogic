...
class Ensemble:
	def __init__(self, method, strategy_names, weights):
		try:
			fw = open("strategies/weights_%s.txt" % method, "r")
			self.my_weights_ = simplejson.loads(fw.read())
			fw.close()

			fs = open("strategies/strategies_%s.txt" % method, "r")
			strategy_stored_names = simplejson.loads(fs.read())
			fs.close()

			fn = open("strategies/normalized/mean_%s.txt" % method, "r")
			self.scores_mean_ = simplejson.loads(fn.read())
			fn.close()
			
			fst = open("strategies/normalized/stdev_%s.txt" % method, "r")
